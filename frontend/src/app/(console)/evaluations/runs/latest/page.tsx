...
function RunContent({ data }: { data: EvaluationViewModel }) {
  return (
    <div className="space-y-[22px]">
      ...
      <QualityCard tier="secondary"><h2 className="text-base font-semibold text-neutral-900">Regenerate the artifact</h2><p className="mt-2 text-sm text-neutral-600">The browser displays read-only evaluation artifacts. Benchmark execution is started explicitly from the repository.</p><pre className="mt-4 overflow-x-auto rounded-lg border border-neutral-200 bg-neutral-50 p-4 font-mono text-xs text-neutral-800"><code>bash scripts/evaluation/run-compose-eval.sh</code></pre><p className="mt-4 text-sm leading-6 text-neutral-600">The helper uses the configured Compose backend port, runs all benchmark cases, and safely installs the completed JSON and CSV artifacts into the existing evaluation-results volume.</p><details className="mt-4 text-sm text-neutral-600"><summary className="cursor-pointer font-medium text-neutral-700">Native-development command</summary><pre className="mt-3 overflow-x-auto rounded-lg border border-neutral-200 bg-neutral-50 p-4 font-mono text-xs text-neutral-800"><code>python eval/run_eval.py --base-url http://localhost:8000 --dataset eval/questions.jsonl --output-dir eval/results</code></pre></details></QualityCard>
    </div>
  );
}
