...
import { evaluationLabel, formatEvaluationPercent } from "@/lib/formatters/evaluation";
  ...
  return (
    <div className="space-y-[22px]">
      ...
      <div className="grid gap-[22px] sm:grid-cols-2 xl:grid-cols-4">
        <QualityCard tier="outcome" label="Observed generation mode" value={degraded ? "Citation-only fallback" : "Generated answers"} interpretation={degraded ? "Evidence retrieval completed, but no generated answer was returned." : "Generated answers were returned by the observed provider."} status={degraded ? "warning" : "success"} />
      ...
      <QualityCard tier="secondary"><h2 className="text-base font-semibold text-neutral-900">Generation error categories</h2>{stats.errorCategories.length ? <ul className="mt-4 space-y-2">{stats.errorCategories.map((item) => <li key={item.name} className="flex items-center justify-between gap-3 border-t border-neutral-200 pt-2 first:border-0 first:pt-0"><span className="text-sm text-neutral-700">{item.name === "provider_generation_failure" ? "Provider unavailable fallback" : evaluationLabel(item.name)}</span><span className="font-metric text-sm font-semibold text-neutral-900">{item.count}</span></li>)}</ul> : <p className="mt-3 text-sm text-neutral-600">No provider generation failures were recorded.</p>}</QualityCard>
    </div>
  );
}
